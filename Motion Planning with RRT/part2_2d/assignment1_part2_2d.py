...
class RRT():
    ...
    def __init__(self, start, goal, obstacleList, randArea, alg, geom, dof=2, expandDis=0.05, goalSampleRate=5, maxIter=100):
        ...
        self.rectanglerad = math.sqrt(0.75**2 + 1.5**2)
        self.vertextheta = math.atan(0.5)
    ...
    def choose_parent(self, newNode, nearinds):
        ...
        for i in nearinds:
            cur = self.nodeList[i]
            tempcost = dist(newNode.state,cur.state)
    ...
    def find_near_nodes(self, newNode):
        ...
        for idx in range(i):
            cur = self.nodeList[idx]
            if dist(cur.state,newNode.state) <= GAMMA*(math.log(i,2)/i)**(1/self.dof):
                nearV.append(idx)
    ...
    def rewire(self, newNode, newNodeIndex, nearinds):
        ...
        for idx in nearinds:
            ...
            flag, newcost = self.steerTo(newNode, cur)

            if flag:
                newcost += cur.cost
                if newcost < newNode.cost:
                    self.nodeList[newNode.parent].children.remove(newNodeIndex)
                    newNode.parent = idx
                    self.nodeList[idx].children.add(newNodeIndex)
    ...
    def __CollisionCheck(self, node):
        ...
        if self.geom == 'rectangle':
            verticiesangle = [-self.vertextheta, self.vertextheta, self.vertextheta + math.pi,
                              math.pi - self.vertextheta]

            for vtheta in verticiesangle:
                x = self.rectanglerad * math.cos(vtheta - node.theta)+node.state[0]
                y = self.rectanglerad * math.sin(vtheta - node.theta)+node.state[1]
            ...
            for j in range(self.dof):
                if abs(obs[j] - s[j]) > obs_size[j] / 2.0:
                    ...
                    if self.geom == 'circle':
                        ...
                        for i in range(n):
                            theta = math.pi * 2 / n * i
                            nx = math.cos(theta) + node.state[0]
                            ny = math.sin(theta) + node.state[1]

                            if nx < self.minrand or nx > self.maxrand:
                                return False

                            if ny < self.minrand or ny > self.maxrand:
                                return False

                            if abs(obs[0] - nx) <= obs_size[0] / 2.0:
                                if abs(obs[1] - ny) <= obs_size[1] / 2.0:
                                    flag = False
                                    break
                        if flag == False:
                            cf = False
                        ...
                        for i in range(4):
                            if verticies[i][0] < self.minrand or verticies[i][0] > self.maxrand:
                                return False
                            if verticies[i][1] < self.minrand or verticies[i][1] > self.maxrand:
                                return False

                            for k in range(n + 1):
                                nx = (verticies[i - 1][0] - verticies[i][0]) / n * k + verticies[i][0]
                                ny = (verticies[i - 1][1] - verticies[i][1]) / n * k + verticies[i][1]
            ...
            if node.parent is not None:
                if node.state is not None:
                    if self.geom == 'circle':
                        ...
                        for i in range(n):
                            theta = math.pi * 2 / n * i
                            thetan = math.pi * 2 / n * (i+1)
                            plt.plot([math.cos(theta) + node.state[0], math.cos(thetan) + node.state[0]], [
                                math.sin(theta) + node.state[1], math.sin(thetan) + node.state[1]], "-k")
                    elif self.geom == 'rectangle':
                        verticiesangle = [self.vertextheta, -self.vertextheta, self.vertextheta + math.pi,
                                          math.pi - self.vertextheta]

                        for i in range(4):
                            vtheta = verticiesangle[i]
                            nvtheta = verticiesangle[i-1]
                            x = self.rectanglerad * math.cos(vtheta - node.theta)+node.state[0]
                            y = self.rectanglerad * math.sin(vtheta - node.theta)+node.state[1]
                            nx = self.rectanglerad * math.cos(nvtheta - node.theta)+node.state[0]
                            ny = self.rectanglerad * math.sin(nvtheta - node.theta)+node.state[1]
